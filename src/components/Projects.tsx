...
const projects = [
  ...
  },
  {
    ...
    tags: ['LiveKit', 'FastAPI', 'PostgreSQL', 'Next.js'],
    description:
      'AI stand-up comedian that performs multi-joke sets over live audio with barge-in, scores listener reactions, and files every joke with a full decision trace. Real-time voice loop on self-hosted LiveKit with Deepgram and Silero VAD at 1.6 s median response latency, runs on Gemini or Claude from a single key, with a FastAPI + Postgres archive and 96 tests under CI.',
  ...
  },
  {
    ...
    tags: ['Playwright', 'Pydantic', 'FastAPI', 'LLM APIs'],
    description:
      'An LLM discovers a workflow on a legacy, API-less banking UI once, then it is compiled into a typed, versioned artifact that replays deterministically with no model in the loop at zero cost. Text-based DOM perception survives relabeled tenants, human-in-the-loop handoff runs on a shared live browser session, and safety is enforced in code with allowlists, risk classification, and redaction. 131 automated tests.',
    ...
    gradientTo: '#155e75',
    accentColor: '#22d3ee',
    label: 'DISCOVER ONCE, REPLAY FOREVER',
    isResearch: false,
